...
- `isolate=true` (alias `static_ip`): EC2+EIP+Caddy with Redis plus **MySQL or Postgres on the same host** (not the shared pool). Pass `isolate_db=mysql` (default) or `isolate_db=postgres` (`pgsql`/`postgresql`/`postgre` also work). `database=true` on isolate still means dedicated RDS instead of the host DB.
