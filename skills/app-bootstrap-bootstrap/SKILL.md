...
- `database=true`: on **shared-dev** → shared Postgres EC2; on **isolate-dev** → host Docker MySQL/Postgres + Redis (never RDS); on **prod** → dedicated RDS. Optional `db_instance_class` (default `db.t4g.micro`).
