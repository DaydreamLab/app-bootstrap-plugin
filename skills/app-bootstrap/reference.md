...
Server name: `app-bootstrap`。`repo` 從 `git remote` → `owner/name`。預設 `env=dev`。空 `region` 用平台預設（`ap-southeast-1`）。
...
Auth: Cursor OAuth to the MCP URL. No static Bearer in mcp.json.
